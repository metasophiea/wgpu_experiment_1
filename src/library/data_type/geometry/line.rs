...
//new
    impl Line {
        ...
        pub fn new_from_point_vector(points:Vec<Point>) -> Line {
            if points.len() < 2 {
                println!("WARNING - Line::new_from_point_vector - supplied vector doesn't have enough points");
                return Line::new_empty();
            } else if points.len() > 2 {
                println!("WARNING - Line::new_from_point_vector - supplied vector has too many points");
                return Line::new_empty();
            }
            Line {
                point_a: points[0],
            ...
            //parallel lines
                ...
                if denominator == 0.0 { //indicates that the two lines are parallel
                    ...
                    return if range.len() == 0 { //no touching
                        ...
                        println!("WARNING - Line::intersect_with_line - line overlap, but more than 2 points found?");
